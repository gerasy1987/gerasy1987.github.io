...
- **Link previews** (WhatsApp, Slack, Twitter, and so on): the preview title is the `open-graph` and `twitter-card` `title` in the front matter of `index.qmd`, the description is the `description` there and in `_quarto.yml`, and the picture is `image` in `_quarto.yml`, which must be a square image well under 300 KB. Messaging apps cache previews, so a link shared before a change can keep showing the old preview for a while.
