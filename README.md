...
- **Link previews** (WhatsApp, Slack, Twitter, and so on): the preview title is the `open-graph` and `twitter-card` `title` in the front matter of `index.qmd`, the description is the `description` there and in `_quarto.yml`, and the picture is `image` in `_quarto.yml`, which must be a square image well under 300 KB. The `og:url` and `og:type` tags are written by hand at the top of `_includes/head.html`, because Quarto's `open-graph` option does not emit them and WhatsApp shows no preview at all without `og:url`, so keep them there. Messaging apps cache previews, so a link shared before a change can keep showing the old preview for a while. To see a fresh fetch, share the link with a throwaway query string such as `?v=2`.
